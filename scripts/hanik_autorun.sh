...
7. 이 프로세스를 시작한 부모 `scripts/hanik_autorun.sh`는 정상적인 오케스트레이터다. 작업 디렉터리가 같아도 별도 세션이 파일을 덮어쓰는 고장이 아니다. 러너 자체를 관리하려 하지 마라. `scripts/hanik_autorun.sh stop`·`finish`를 실행하거나 러너 PID를 종료하지 말라. 이 세션이 끝나면 부모 러너가 다음 세션을 시작하는 것이 정상이며, 작업 파일을 보존하려고 부모 러너를 멈추면 안 된다.
8. 작업 파일을 보존하려면 러너를 멈추는 대신 이 세션에서 지정된 검증과 `src.hanik_loop` 실행을 끝까지 수행하라. 부모 프로세스의 PID를 조사하거나 종료하는 일은 이번 과제가 아니며 절대로 하지 말라.
PROMPT
)" \
        --mode autopilot \
        --model "$MODEL" \
