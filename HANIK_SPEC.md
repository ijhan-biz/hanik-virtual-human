...
- **R13은 예산을 넘긴 구획이 있을 때만 문다.** 조건 하나의 실질 분량 상한은
  `HANIK_CONDITION_BUDGET`(기본 6000자), 서문은 `HANIK_PREAMBLE_BUDGET`(기본 4000자)
  이다. 어느 하나라도 넘으면 그 반복은 **정리 모드**가 되고, 문서 전체의 실질 분량이
  직전 반복보다 줄어야 통과한다. 예산 안에 있으면 R13은 아무것도 요구하지 않는다.
  비교할 직전 분량이 아직 기록되지 않았으면 통과시킨다.
...
이것은 가설이 아니라 관측이다. 610번의 반복 뒤 `Hanik.md`는 조건 세 개에 42만 자가
...
| `정체` | 최근 `HANIK_STAGNATION_LIMIT`(기본 5)번의 반복이 같은 증거 서명 | 다시 돌려도 달라지지 않는다 |
...
- **정리가 실제로 추려낸 것인지.** R13은 분량이 줄었다는 것까지만 안다. 핵심 논증을
  ...
  한다.
- **예산이 무르게 늘어났는지.** `HANIK_CONDITION_BUDGET`을 올리는 것은 R13을 끄는
